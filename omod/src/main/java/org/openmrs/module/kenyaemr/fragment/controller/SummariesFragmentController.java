...
public class SummariesFragmentController {
    ...
    public void controller(@FragmentParam("patient") Patient patient,
        ...
        if(upn.size() > 0){
            ...
            //patient entry point
            CalculationResultMap ftPlanning = Calculations.lastObs(Dictionary.getConcept(Dictionary.METHOD_OF_FAMILY_PLANNING), Arrays.asList(patient.getPatientId()), context);
            Obs ftPlanningObs = EmrCalculationUtils.obsResultForPatient(ftPlanning, patient.getPatientId());
            if(ftPlanningObs != null) {
                patientSummary.setFamilyProtection(entryPointAbbriviations(ftPlanningObs.getValueCoded()));
            }
            else {
            ...
            //CaCX
            CalculationResultMap cacxMap = Calculations.lastObs(Dictionary.getConcept(Dictionary.CACX_SCREENING), Arrays.asList(patient.getPatientId()), context);
            Obs cacxObs = EmrCalculationUtils.obsResultForPatient(cacxMap, patient.getPatientId());
            if(cacxObs != null) {
                patientSummary.setCaxcScreeningOutcome(entryPointAbbriviations(cacxObs.getValueCoded()));
            }
            else {
            ...
            //STI SCREENING
            CalculationResultMap stiScreen = Calculations.lastObs(Dictionary.getConcept(Dictionary.STI_SCREENING), Arrays.asList(patient.getPatientId()), context);
            Obs stiObs = EmrCalculationUtils.obsResultForPatient(stiScreen, patient.getPatientId());
            if(stiObs != null) {
                patientSummary.setStiScreeningOutcome(entryPointAbbriviations(stiObs.getValueCoded()));
            }
            else {
            ...
            if(tbObs != null) {
                patientSummary.setTbScreeningOutcome(entryPointAbbriviations(tbObs.getValueCoded()));
            }
            else {
...
>>>>>>> ea2c57842... KHP3-285: KNH/KDO patient summary
        ...
        //most recent cd4
           ...
           //transfer out to facility
            ...
            model.addAttribute("familyProtection", patientSummary.getFamilyProtection());
