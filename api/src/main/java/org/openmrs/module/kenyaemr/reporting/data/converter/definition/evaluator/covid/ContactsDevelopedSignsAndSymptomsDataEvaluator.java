...
public class ContactsDevelopedSignsAndSymptomsDataEvaluator implements PersonDataEvaluator {
    ...
    public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context) throws EvaluationException {
        ...
        String qry = "select case_patient, count(*) from (\n" +
                "select patient_id, case_patient, coalesce(first_gov_quarantine_date, first_self_q_followup_date) as followupStartDate, coalesce(last_gov_quarantine_date, last_self_q_followup_date) as followupEndDate, cough, fever, difficulty_breathing from (\n" +
                "select c.patient_id, c.patient_related_to case_patient, min(self_q.visit_date) first_self_q_followup_date, min(gov_q.visit_date) first_gov_quarantine_date, max(self_q.visit_date) last_self_q_followup_date, max(gov_q.visit_date) last_gov_quarantine_date, coalesce(gov_q.cough, self_q.cough) cough, coalesce(gov_q.fever, self_q.fever) fever, coalesce(gov_q.difficulty_breathing, self_q.difficulty_breathing) difficulty_breathing\n" +
                ...
                "      ) f \n" +
                "having datediff(followupEndDate,followupStartDate) < 14 and (fever='Yes' or cough='Yes' or difficulty_breathing='Yes') ) a\n" +
